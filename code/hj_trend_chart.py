...
def hj_trend_chart2(dataframe):
    ...
    #for i in df:
    #    week_df.append(make_week_num(i))
    month_df = make_month_num(df)
    week_df  = make_week_num(df)
    ...
    #         name='Cate' + str(i),
    #         line=dict(width=1.5)))
    month_scatter_plot = go.Scatter(x=list(month_df['month_num']), y=list(month_df['count']))
    layout = go.Layout(
        font={
            "family": "뫼비우스 Regular",
            "size": 15
    ...
    week_scatter_plot  = go.Scatter(x=list(week_df[i]['week_num_new']), y=list(week_df[i]['count']))
    layout = go.Layout(
        font={
            "family": "뫼비우스 Regular",
            "size": 15
